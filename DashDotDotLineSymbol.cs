...
namespace CMA.MICAPS.Symbols
{
    ...
    class DashDotDotLineSymbol : LabelLineSymbol
    {
        private List<int> _point_indices;
        private int _time;
        private int _dash_len;
        private int _dash_interval;
        public DashDotDotLineSymbol(Line line, int dash_len, int dash_interval)
            : base(line)
        {
        ...
        public DashDotDotLineSymbol(Line line, string label, uint size, System.Drawing.Color label_color, LabelPosition position, bool rotation, int dash_len, int dash_interval, bool split = false)
        ...
        public void PrepareForDraw(Box2D.Graphics.Context context)
        {
            ...
            for (int i = 0; i < pt_num; i += (3 * dash_interval + dash_len))
            {
                int flag = 0;
                _indices.Add(i);
                ...
                if (i + 2 * dash_interval + dash_len < pt_num)
                    _point_indices.Add(i + 2 * dash_interval + dash_len);
            }

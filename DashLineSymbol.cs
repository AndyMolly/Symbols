...
namespace CMA.MICAPS.Symbols
{
    ...
    class DashLineSymbol : LabelLineSymbol
    {
        private int _time;
        private int _dash_len;
        private int _dash_interval;
        public DashLineSymbol(Line line, int dash_len, int dash_interval)
            : base(line)
        {
        ...
        public DashLineSymbol(Line line, string label, uint size, System.Drawing.Color label_color, LabelPosition position, bool rotation, int dash_len, int dash_interval, bool split = false)
        ...
        public override void Render(Box2D.Graphics.SceneManager scene, Box2D.Graphics.Context context)
        {
            ...
            StaticBufferDrawHelper.DrawIndex(_vertices.ToArray(), _indices.ToArray(), this.Material.SurfaceState.color, PrimitiveType.Lines, _indices.Count);
        }
        public void PrepareForDraw(Box2D.Graphics.Context context)
        {
            ...
            for (int i = 0; i < pt_num; i += (dash_interval + dash_len))
            {
                int flag = 0;
                _indices.Add(i);
